...
class MAMCA(nn.Module, GenerationMixin):
    def __init__(self, config: MambaConfig, length=1000, num_claasses=2, device=None, dtype=None, fused_add_norm=False):
        super().__init__()
        self.config = config
        ...
        # Denosing unit
        self.denosing = denosing_unit(BasicBlock, 2, in_channel=1, out_channel=config.d_model)

        # Classification head
        self.fc = nn.Sequential(
            nn.Dropout(0.3),
            nn.Linear(int(self.config.d_model * length), num_claasses)
        )
...
def get_model(input_length, num_classes, device="cuda"):
    ...
    config.n_layer = 1  # 层数
    config.ssm_cfg = {"d_state": 8, "d_conv": 4, "expand": 2}  # SSM 参数
