...
class MambaBlock(torch.nn.Module):
    def __init__(
        ...
        # S4D Initialization
        A = torch.arange(1, self.latent_state_dim + 1, dtype=torch.float).repeat(
            self.expanded_dim, 1
        )
    ...
    def forward(self, x):
        ...
        if x.dim() == 2:  # if input shape is [batch_size, sequence_length]
            x = x.unsqueeze(-1)  # make it [batch_size, sequence_length, 1]
...
class MambaBEAT(torch.nn.Module):
    def __init__(
        self,
        in_channels: int,  # Adjust in_channels to 1 for 1-lead data
        out_channels: int,
        n_layers: int = 1,
        ...
        # Initialize layers with adjusted input dimensions
