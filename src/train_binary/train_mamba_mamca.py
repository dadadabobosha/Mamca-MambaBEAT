...
# DATA_PATH = os.path.abspath(os.path.join(current_folder, "..", "..", "test_data", "10k"))  # test data path in local
...
def main() -> None:
    ...
    optimizer: torch.optim.Optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=5e-3)
    ...
    try:
        for epoch in tqdm(range(epochs)):
            ...
            print(f"Epoch {epoch} | Loss: {train_loss:.4f} | Accuracy: {train_accuracy:.4f} | F1: {train_f1:.4f} | Train Time: {epoch_train_time:.4f} seconds")
            ...
            print(f"Epoch {epoch} | Val Loss: {val_loss:.4f} | Val Accuracy: {val_accuracy:.4f} | Val F1: {val_f1:.4f}")
    ...
    plot_metrics(train_losses, val_losses, train_accuracies, val_accuracies, train_f1_scores, val_f1_scores,
                 current_time)
...
def plot_metrics(train_losses, val_losses, train_accuracies, val_accuracies, train_f1_scores, val_f1_scores,
                 current_time, name="Mamca", N_CLASSES=2, lr=1e-2):
    """
